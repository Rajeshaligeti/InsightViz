...
This project includes configuration for [Vercel](https://vercel.com) serverless deployment.

- **Config**: `vercel.json` routes all requests to `api/wsgi.py`, which exposes the Django WSGI app as `app` (required by Vercel's Python runtime).
- **Environment variables** (optional in Vercel dashboard):
  - `SECRET_KEY` – set a secure key in production (recommended).
  - `DEBUG` – set to `False` in production.
  - `ALLOWED_HOSTS` – comma-separated list if you use a custom domain.
- **Limitations on Vercel**: The serverless filesystem is read-only except `/tmp`. The SQLite database and uploaded files use `/tmp`, so they are **ephemeral** (reset on cold starts/redeploys). For persistent data, use an external database and object storage.
